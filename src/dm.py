...
def dm_leave_v1(token, dm_id):
    ...
    for dm in data['dms']:
        if dm['dm_id'] == dm_id:
            for users in data['users']:
                if users['u_id'] == user:
                    dm['members'].remove(users)
                    handlelist = dm['name'].split(",")
                    handlelist.remove(users['handle_str'])
                    break
            handlelist.sort()
            handlelist_string = ', '.join(map(str, handlelist))
            dm['name'] = handlelist_string        
