...
@pytest.fixture
def auth_id1():
    auth_id1 = auth_register_v1("Godan@gmail.com", "password", "Godan", "Liang")
    return auth_id1
    
@pytest.fixture
def auth_id2():
    auth_id2 = auth_register_v1("Jeremy@gmail.com", "password", "Jeremy", "Lee")
    return auth_id2

    
@pytest.fixture
def auth_id3():
    auth_id3 = auth_register_v1("Roland@gmail.com", "password", "Roland", "Lin")
    return auth_id3
    
@pytest.fixture
def auth_id_unadded():
    auth_id_unadded = auth_register_v1("Bolin@gmail.com", "password", "Bolin", "Ngo")
    return auth_id_unadded

@pytest.fixture
def user_token1():
    user_token1 = auth_id1
    return user_token1
    
@pytest.fixture
def user_token2():
    user_token2 = auth_id2
    return user_token2
    
@pytest.fixture
def user_token3():
    user_token3 = auth_id3
    return user_token3

@pytest.fixture
def unadded_user_token():
    user_token = auth_id_unadded
    return user_token
...
def test_dm_invite_v1_InputError_1(clear_data, user_token1, auth_id2):
    """"
    InputError happnes when dm_id does not refer to a existing DM
    """"
    with pytest.raises(InputError):
        assert dm_invite_v1(user_token1['token'], 69, auth_id2)
...
def test_dm_invite_v1_AccessError(clear_data, unadded_user_token, dm_2, auth_id3):
    """"
    AccessError happens when authorised user is not already a member of the DM
    """"
    with pytest.raises(AccessError):
        assert dm_invite_v1(unadded_user_token['token'], dm_2['dm_id'], auth_id3)
        
def test_dm_invite_v1_Invite1(clear_data, user_token1, dm_2, auth_id3):
    """"
    Test whether user will be invited and added to the specified DM
    """"
    dm_invite_v1(user_token1['token'], dm_2['dm_id'], auth_id3)
    dm_details = dm_details_v1(user_token1['token'], dm_2['dm_id'])
    assert dm_details['members'][-1]['u_id'] == auth_id3
    
def test_dm_invite_v1_Invite2(clear_data, user_token1, dm_2, auth_id3, auth_id_unadded):
    """"
    Test whether two users can be invited and added to the specified DM properly
    """"
    dm_invite_v1(user_token1['token'], dm_2['dm_id'], auth_id3)
    dm_invite_v1(user_token1['token'], dm_2['dm_id'], auth_id_unadded)
    dm_details = dm_details_v1(user_token1['token'], dm_2['dm_id'])
    assert dm_details['members'][-2]['u_id'] == auth_id3
    assert dm_details['members'][-1]['u_id'] == auth_id_unadded
...
def test_dm_leave_v1_Leave1(clear_data, user_token1, dm_2, auth_id3, user_token3):
    """"
    ...
    dm_details = dm_details_v1(user_token1['token'], dm_2['dm_id'])
    assert dm_details['members'][-1]['u_id'] == auth_id2
    
def test_dm_leave_v1_LeaveAll(clear_data, user_token1, dm_2, auth_id3, user_token3):
    """"
...
def test_dm_messages_v1_AccessError(clear_data, unadded_user_token, dm_1):
    ...
    """"
    with pytest.raises(AccessError):
        assert dm_messages_v1(unadded_user_token['token'], dm_1['dm_id'])
